...
int html_parser_cmp_tag(HTMLSTREAMPARSER *hsp, const char *p, size_t l) { if (html_parser_tag_length(hsp) == l) if (strncmp(p, hsp->tag_name, l) == 0) return 1; return 0; }
...
int html_parser_cmp_attr(HTMLSTREAMPARSER *hsp, const char *p, size_t l) { if (html_parser_attr_length(hsp) == l) if (strncmp(p, hsp->attr_name, l) == 0) return 1; return 0; }
...
int html_parser_cmp_val(HTMLSTREAMPARSER *hsp, const char *p, size_t l) { if (html_parser_val_length(hsp) == l) if (strncmp(p, hsp->attr_value, l) == 0) return 1; return 0; }
...
int html_parser_cmp_inner_text(HTMLSTREAMPARSER *hsp, const char *p, size_t l) { if (html_parser_inner_text_length(hsp) == l) if (strncmp(p, hsp->inner_text, l) == 0) return 1; return 0; }
